  ...
  try {
    ...
    push(React.createElement(
      ...
      ...repoGroups.map((group) => React.createElement(
        ...
        React.createElement(Text, { color: '#F9FAFB', bold: true }, `${group.owner}/${group.repo} (${group.pullRequests.length} PRs)`),
      ...
      React.createElement(Text, { color: '#6B7280' }, `Total: ${total} open PRs across ${repoGroups.length} repos`)
    ));
  } catch (error) {

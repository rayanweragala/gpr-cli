...
async function findPullRequestByBranchWithState(api, owner, repo, branch, state) {
  const pullRequests = await listPaginated(api, `/repos/${owner}/${repo}/pulls`, {
    state: state || 'open'
  });
...
async function listPullRequests(api, owner, repo, state) {
  if (state === 'all') {
    const [open, closed] = await Promise.all([
      listPaginated(api, `/repos/${owner}/${repo}/pulls`, { state: 'open' }),
      listPaginated(api, `/repos/${owner}/${repo}/pulls`, { state: 'closed' })
    ]);

    return [...open, ...closed];
  }

  return listPaginated(api, `/repos/${owner}/${repo}/pulls`, {
    state: state || 'open'
  });
}

async function listBranches(api, owner, repo) {
  return listPaginated(api, `/repos/${owner}/${repo}/branches`);
}
...
async function mergePullRequest(api, owner, repo, number, payload) {
  const response = await withRetry(() => api.put(
    `/repos/${owner}/${repo}/pulls/${number}/merge`,
    {
      merge_method: payload && payload.merge_method ? payload.merge_method : 'merge',
      commit_title: payload && payload.commit_title ? payload.commit_title : '',
...
async function closePullRequest(api, owner, repo, number) {
  const response = await withRetry(() => api.patch(
...
async function reopenPullRequest(api, owner, repo, number) {
  const response = await withRetry(() => api.patch(
    `/repos/${owner}/${repo}/pulls/${number}`,
    { state: 'open' }
  ));
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      process.stderr.write(
        `[collaborators error] ${err.response?.status} ${err.config?.url} ${err.message}\n`
      );
    }
  }
...
async function getOrgMembers(api, org) {
  try {
    ...
    process.stderr.write(
      `[org members error] ${err.response?.status} ${err.config?.url} ${err.message}\n`
    );
    return [];
  }
}
...
async function createIssueComment(api, owner, repo, number, body) {
  const response = await withRetry(() => api.post(
...
async function listPaginated(api, url, params) {
  const perPage = 100;
  const maxPages = 20;
  const rows = [];
  ...
  for (let page = 1; page <= maxPages; page += 1) {
    ...
    const signature = `${batch.length}:${String(batch[0].id || batch[0].name || batch[0].number || '')}`;
    if (page > 1 && signature === lastPageSignature) {
      break;
    }
